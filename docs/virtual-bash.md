...
`@verifs/virtual-bash` is a small deterministic command interpreter for agents. It does not execute host shell commands.

```ts
import { createVeriFSShell } from "@verifs/virtual-bash";

const shell = createVeriFSShell({
  verifs,
  workspaceId,
  actor: "agent:demo"
});
...
- Paths are normalized by VeriFS and `../` traversal is rejected.
