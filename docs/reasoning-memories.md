...
They are not raw chain-of-thought. VeriFS stores concise, source-backed reasoning summaries: the situation, the strategy, the failure pattern, the success pattern, and when the lesson should be recalled again.
...
If an LLM is configured, VeriFS asks it for strict JSON and validates the output before storing anything. If no model is configured, deterministic extraction looks for failures, successes, strategies, and follow-up patterns in `result.md`, `errors.md`, `followups.md`, `actions.md`, and `memory-used.md`.
