...
Pass `reasoning: true` or run `memfs run compile <run_id> --reasoning` to also write `/runs/<run_id>/reasoning-memories.json` and create reviewable `reasoning_memory` candidates. Reasoning candidates capture reusable lessons from successes and failures; they are not auto-approved.
